...
| `plan:test` -> `deploy:test` -> `smoke-test:test` | Test | `release/*` branch after `promote:test` | Explicit chained DAG; all test actions are blocked until promotion is approved |
