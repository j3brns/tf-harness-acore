...
variable "acm_certificate_arn" {
  description = "ACM Certificate ARN for the custom domain (must be in eu-central-1)"
  type        = string
  default     = ""
}

variable "cloudfront_waf_acl_arn" {
  description = "ARN of a WAFv2 Web ACL (CLOUDFRONT scope, must be in eu-central-1) to associate with the CloudFront distribution. Leave empty to disable WAF on CloudFront (default). Required format: arn:aws:wafv2:eu-central-1:<account>:global/webacl/<name>/<id>."
  type        = string
  default     = ""
  validation {
    condition     = var.cloudfront_waf_acl_arn == "" || can(regex("^arn:aws:wafv2:eu-central-1:[0-9]{12}:global/webacl/[^/]+/[^/]+$", var.cloudfront_waf_acl_arn))
    error_message = "cloudfront_waf_acl_arn must be empty or a valid WAFv2 CLOUDFRONT-scope ARN in eu-central-1, e.g. arn:aws:wafv2:eu-central-1:123456789012:global/webacl/my-acl/abc123."
  }
}
