...
# Inference profile ARN (profile is created in agentcore-foundation; ARN passed here for SSM persistence)
variable "inference_profile_arn" {
  description = "Bedrock application inference profile ARN from agentcore-foundation (empty string when not enabled)"
  type        = string
  default     = ""
}
